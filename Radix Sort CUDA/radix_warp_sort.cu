...
// implementation of radix sort using warp level primitives
__global__ void radix_warp_sort()
{
    __shared__ volatile unsigned int s_data[WARP_SIZE * 2];

    // load data from global memory to shared in a coalesced manner
    ...
    for (int i = MIN_BIT_POS; i <= MAX_BIT_POS; i++)
    {
        ...
        // do zeroes, then ones
        if (!bit) // threads with zero bit
        {
            // get position in ping-pong buffer
            pos = __popc(zeroes & thread_mask);
        }
        else// threads with one bit
        {
            // get position in ping-pong buffer
...
void validate(unsigned int* h_data)
{
    for (int i = 0; i < WARP_SIZE - 1; i++)
    {
        ...
        {
            printf("Sorting unsuccessful: h_data[%d] = %d > h_data[%d] = %d\n", i, h_data[i], i + 1, h_data[i + 1]);
            exit(EXIT_FAILURE);
        }
    }
}
